...
class Cminds_Marketplace_Model_Adminhtml_Observer {
    public function onCustomerSave($observer) {
        ...
        try {
            ...
            if(isset($postData['supplier_name']) && $postData['supplier_name_new']) {
            ...
            $url->setRequestPath($urlPath);

            $transaction->addObject($url);
    ...
    private function createRewritedName($customer) {
        ...
        while(true) {
            $rewritedName = 'vendor/' . Mage::helper('marketplace')->clearString($supplier_name) . '-' . $i . '.html';
