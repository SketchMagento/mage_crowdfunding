    ...
    public function saveAction() {
        if($this->_request->isPost()) {
            ...
            try {
                ...
                if($editMode) {
                    Mage::app()->getFrontController()->getResponse()->setRedirect(Mage::getUrl('marketplace/product/editConfigurable/', array('id' =>  $postData['product_id'], 'type' => $postData['type'])));
                } else {
                    Mage::app()->getFrontController()->getResponse()->setRedirect(Mage::getUrl('marketplace/product/createConfigurable/', array('attribute_set_id' =>  $postData['attribute_set_id'], 'type' => $postData['type'])));
                }
            }
        }
    }
