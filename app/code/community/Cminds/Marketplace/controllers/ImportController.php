...
class Cminds_Marketplace_ImportController extends Cminds_Marketplace_Controller_Action {
        ...
        if(isset($_FILES['file']['name']) && ($_FILES['file']['tmp_name'] != NULL)) {
            ...
            if (($handle = fopen($_FILES['file']['tmp_name'], "r")) !== FALSE) {
                if(is_int(Mage::getStoreConfig('marketplace_configuration/csv_import/product_limit')) &&
                    ...
                    Mage::getSingleton('core/session')->addError(Mage::helper('marketplace')->$this->__("Too many products added to import."));
                } else {
                    while (($data = fgetcsv($handle)) !== FALSE) {
                        if($i != 0){
                            ...
                            if(count($s) > 0) {
                                Mage::getSingleton('core/session')->addError(Mage::helper('marketplace')->$this->__("Attributes doesn't match all required attributes. Missing attribute : " . $s[0]));
                                break;
                            }
                            $headers = $data;
                        }
            ...
            if(!$productModel) {
                ...
                $collection = Mage::getResourceModel('eav/entity_attribute_set_collection')->setEntityTypeFilter($entityType);
                     foreach($collection AS $attributeSet){
                            ...
                            Mage::throwException($this->__('Attribute Set don\'t exist'));
                            }
            ...
            foreach($headers AS $i => $header) {
                ...
                if(isset($line[$i])) {
                    ...
                    if($this->_isRequired($attributeCode)) {
                        throw new Exception($this->__("Value for attribute : ") . $attributeCode . $this->__(" is not valid"));
                    }
                }
            }
    ...
    protected function _findProduct($headers, $line) {
        ...
        if(!$product->getId()) throw new Exception($this->$this->__("Product does not exists"));

        if($product->getCreatorId() != Mage::helper('supplierfrontendproductuploader')->getSupplierId()) throw new Exception($this->$this->__("Product does not exists"));
    ...
    private function _validateAttributeValue($attribute_code, $value) {
        ...
        if($attributeModel->getFrontendInput() == 'select') {
            ...
            if(count($availableValues) > 0 || $value != '') {
                if(!in_array($value, $availableValues)) {
                    throw new Exception($this->__("Value of attribute ") . $attribute_code . $this->__(" is not valid"));
                }
            }
        }
        ...
        if($attributeModel->getBackendType() == 'decimal') {
            if(!is_numeric($value)) {
                throw new Exception($this->__("Value of attribute %s is not valid. Should be numeric.", $attribute_code));
            }
        }
    }
